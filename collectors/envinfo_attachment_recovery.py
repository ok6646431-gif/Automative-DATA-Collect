...
The primary ENV-INFO collector historically counted duplicate attachment bytes against a
single 500 MiB run budget. Large companies can attach the same corporate report or policy
to many site-year disclosures, so the budget could be exhausted by duplicate files and
all later attachments were then reported as DOWNLOAD_FAILED.

This recovery stage is intentionally generic:
- deduplicate already downloaded attachments by SHA-256 and keep one physical copy;
- count the storage budget by unique physical bytes, not repeated logical references;
- retry only failures caused by the old total-budget condition;
- recover higher-value evidence first;
- if the unique-byte budget is genuinely exhausted, mark the remaining rows
  SKIPPED_TOTAL_BUDGET rather than pretending the source/download failed.
"""
    ...
    for row in rows:
        for key in row:
            if key not in fields:
                fields.append(key)
    with (out / "attachment_index.csv").open("w", encoding="utf-8-sig", newline="") as f:
...
def deduplicate_downloaded(rows, repo_root="."):
    """Collapse duplicate physical files while preserving every logical attachment row."""
    by_sha = {}
    unique_bytes = 0
    duplicate_rows = 0
    duplicate_bytes = 0
    ...
    return by_sha, unique_bytes, duplicate_rows, duplicate_bytes


def recovery_priority(row):
    """Prefer evidence needed for the human archive/study layer before low-value training files."""
    section = str(row.get("section_id") or "").lower()
    category = str(row.get("document_category") or "")
    importance = str(row.get("importance") or "")
    if section == "inquiry26":  # environment/sustainability report
...
def download_without_total_cap(session, row, attachments_root, max_attempts=2):
    """Download one source attachment with the per-file guard only.

    The caller applies the unique-byte total budget after hashing, because duplicate
    files cannot be identified safely before their payload is received.
    """
    last_exc = None
    for attempt in range(max_attempts):
        ...
        try:
            ...
            r = session.post(
                ...
                timeout=(8, 60), stream=True, allow_redirects=True,
            )
            r.raise_for_status()
...
def recover(out_dir="output/ENVINFO", repo_root=".", total_limit=None, session=None):
    out = Path(out_dir)
    rows = read_rows(out / "attachment_index.csv")
    status_path = out / "status.json"
    status = json.loads(status_path.read_text(encoding="utf-8")) if status_path.exists() else {}
    if not rows:
        return {"recovered": 0, "deduplicated": 0, "skipped_budget": 0, "remaining_failed": 0}

    total_limit = int(total_limit or base.MAX_ATTACHMENT_TOTAL_BYTES)
    by_sha, unique_bytes, duplicate_rows, duplicate_bytes = deduplicate_downloaded(rows, repo_root)
    candidates = [
        r for r in rows
    ...
    recovered = skipped_budget = attempts_total = 0

    for row in candidates:
        try:
            target, count, digest, ctype, attempts = download_without_total_cap(sess, row, attachments_root)
    ...
    (out / "attachment_recovery_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main():
    ...
    args = ap.parse_args()
    recover(args.out, args.repo_root, args.total_limit)
