...
class TestEventAnalysis(unittest.TestCase):
    ...
    def test_definition_change_segments_and_checks_baseline(self):
        ...
        try:
            ev=root/"events.json"
            ev.write_text(json.dumps({"discovery_status":"COMPLETE","events":[{"event_type":"DISCLOSURE_DEFINITION_CHANGE","event_scope":"COMPANY","event_date_start":"2020-01-01","event_title":"definition","source_key":"OFFICIAL_REPORT","source_locator":"official://report#p10","verification_status":"VERIFIED"}]}),encoding="utf-8")
    ...
    def test_duplicate_event_id_only_fills_blank_fields(self):
        ...
        try:
            ...
            integrate_events(root,ev); events=list(csv.DictReader((root/"Event_Registry.csv").open(encoding="utf-8-sig")))
    ...
    def test_analysis_index_is_reference_only(self):
        ...
        try:
            ...
            build_analysis_index(root); idx=list(csv.DictReader((root/"Analysis_Ready_Index.csv").open(encoding="utf-8-sig")))
            ...
            raw=list(csv.DictReader((out/"discovery.csv").open(encoding="utf-8-sig"))); self.assertEqual(raw[0]["official_total"],"999")
        finally: td.cleanup()
