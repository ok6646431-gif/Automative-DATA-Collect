...
QUESTION_FIELDS=[
    'question_id','review_id','domain','site_name','question_type','question','needed_evidence','status'
]
...
def site_allowed(site_id, scope):
    ...
    if mode!='SITE_SET': return True
    targets=set(scope.get('target_canonical_site_ids') or [])
    return not site_id or site_id in targets or site_id=='MULTI_SITE'


def layer_row(layer,domain,site_id,site_name,time_key,title,statement,source_key,source_locator,state,boundary,*idparts):
    return {
        'evidence_id':stable_id('EVD_',layer,domain,site_id,time_key,title,*idparts),
        'layer':layer,'domain':domain,'canonical_site_id':site_id or '', 'site_name':site_name or '',
        'time_key':time_key or '', 'title':title or '', 'statement':statement or '',
        'source_key':source_key or '', 'source_locator':source_locator or '',
...
def observed_layer(pkg, scope):
    out=[]
    signals={r.get('metric_id'):r for r in read_csv(pkg/'Review_Signal_Registry.csv')}
...
def industry_reference_layer(pkg, semantic_path=None):
    out=[]
    idx=read_csv(pkg/'output'/'CORP_DOCS'/'document_index.csv')
    for d in idx:
    ...
    if semantic_path and Path(semantic_path).exists():
        payload=read_json(semantic_path,{}) or {}
        ...
        if str(payload.get('request_id') or '')==str(profile.get('request_id') or ''):
            for f in payload.get('facts',[]) or []:
                ...
                domain=str(f.get('domain') or 'CROSS_MEDIA')
                out.append(layer_row('INDUSTRY_TECHNICAL',domain,str(f.get('canonical_site_id') or ''),str(f.get('site_name') or ''),str(f.get('time_key') or f.get('year') or ''),str(f.get('title') or ''),str(f.get('statement') or ''),str(f.get('source_key') or ''),str(f.get('source_locator') or ''),'SEMANTIC_FACT',str(f.get('interpretation_boundary') or 'Industry reference only; company application requires company-specific evidence.'),str(f.get('fact_id') or '')))
    return out


def compatible(topic,evidence):
    td=topic.get('domain',''); ed=evidence.get('domain','')
    domain_ok=ed in {td,'CROSS_MEDIA'} or td=='CROSS_MEDIA'
    if not domain_ok: return False
...
def build_cross_candidates(pkg,layers,scope):
    ...
    for t in read_csv(pkg/'Review_Topic_Candidates.csv'):
        cid=t.get('canonical_site_id','')
        if not site_allowed(cid,scope): continue
        matches={layer:[e for e in by_layer[layer] if compatible(t,e)] for layer in ['OBSERVED','COMPANY_ACTION','INDUSTRY_TECHNICAL','FUTURE_DIRECTION']}
        obs=matches['OBSERVED']; actions=matches['COMPANY_ACTION']; industry=matches['INDUSTRY_TECHNICAL']; future=matches['FUTURE_DIRECTION']
        ...
        rows.append({
            ...
            'evidence_layers':'|'.join(present),'why_review':why+'.','limitations':'Layer overlap is review context only; production/flow/permit/operating denominators and site-specific technology may still be required.','human_decision':'UNREVIEWED'
        })
        ...
        if not actions: q('COMPANY_ACTION_GAP','What company/site management action is publicly confirmed for this observed topic?','Official company action, ENVINFO investment, permit or verified event evidence')
...
def run_cross_layer_review(package_root,semantic_path=None,protocol_path=None):
    ...
    (pkg/'Cross_Layer_Review_Summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
    return summary
