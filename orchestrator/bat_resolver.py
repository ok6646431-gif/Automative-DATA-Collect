...
def _identity_maps(pkg):
    ...
    for row in read_csv(Path(pkg)/'Source_Identity.csv'):
        ...
        source=str(row.get('source_key') or ''); sid=str(row.get('source_site_id') or ''); cid=str(row.get('canonical_site_id') or '')
...
def _industry_fields(row):
    return [str(v) for k,v in row.items() if v not in (None,'') and any(m in str(k or '').lower() for m in INDUSTRY_KEY_MARKERS)]
...
def _process_fields(row):
    ...
    for key,value in row.items():
        ...
        lk=str(key or '').lower()
        if lk in IDENTITY_KEYS or any(marker in lk for marker in INDUSTRY_KEY_MARKERS): continue
...
def _term_hits(terms,text):
    low=norm(text); return [term for term in terms if norm(term) and norm(term) in low]
...
def _active_entries(catalog):
    """Only preferred/current revisions participate in company matching.
    Superseded physical publications remain in the master catalog for audit and discovery.
    """
...
def resolve(pkg,catalog_path=CATALOG_PATH,as_of=None):
    ...
    for entry in entries:
        for cid in site_ids:
            ...
            elif future: state='FUTURE_TECHNICAL_CANDIDATE'
...
if __name__=='__main__':
    ...
    a=ap.parse_args(); d=date.fromisoformat(a.as_of) if a.as_of else None; print(json.dumps(resolve(a.package,a.catalog,d),ensure_ascii=False,indent=2))
