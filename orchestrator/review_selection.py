...
def metric_selection(metrics,actions,protocol,scope):
    ...
    for k,rs in sorted(g.items()):
        source,sid,cid,cname,sub,domain,metric,note=k; vals=sorted((r['year'],r['value']) for r in rs if r.get('year') is not None and r.get('value') is not None); ys=sorted(set(y for y,x in vals)); s=series_signal(vals,min_obs); comp='TREND_ELIGIBLE' if len(ys)>=min_obs else ('CONTEXT_ONLY' if len(ys)>=2 else 'EVIDENCE_ONLY'); allowed=scope_allows(scope,source,sid,cid); acts=sorted(set(amin[(cid,domain)])) if allowed else []; level='OVERVIEW' if ys else 'EVIDENCE'; reason=['available_metric'] if ys else []
        ...
        if allowed:
            plan.append({'object_id':mid,'object_type':'METRIC','domain':domain,'site_name':cname,'label':metric,'display_level':level,'selection_reason':'|'.join(reason),'evidence_dimensions':'numeric_signal' if level=='MAIN' and s not in {'MIXED_OR_STABLE','SHORT_SERIES'} else '','related_action_ids':'|'.join(acts),'scope_label':scope.get('label',''),'human_decision':'UNREVIEWED'})
    return inv,sig,plan

def topic_candidates(inv,sig,actions,chems,scope):
...
def run_review_selection(source_root,package_root,protocol_path=None):
    ...
    topics=topic_candidates(inv,sig,actions,chems,scope); daily=daily_stats(root,idmap)
    ...
    summary={'schema_version':'1.1','protocol_version':protocol.get('schema_version'),'scope_mode':scope.get('mode'),'scope_label':scope.get('label'),'metric_rows':len(metrics),'metric_inventory':len(inv),'metric_inventory_in_scope':sum(r['in_requested_scope']=='YES' for r in inv),'signals':len(sig),'signals_in_scope':sum(r['in_requested_scope']=='YES' for r in sig),'management_actions':len(actions),'management_actions_in_scope':sum(r.get('in_requested_scope')=='YES' for r in actions),'prtr_chemical_rows':len(prtr),'prtr_chemical_rows_in_scope':len(prtr_scope),'chem_stats_substance_rows':len(stats),'chem_stats_substance_rows_in_scope':len(stats_scope),'chemical_candidates':len(chems),'topic_candidates':len(topics),'deep_dive_candidates':sum(t['candidate_state']=='DEEP_DIVE_CANDIDATE' for t in topics),'daily_stat_rows':len(daily),'daily_stat_rows_in_scope':sum(r['in_requested_scope']=='YES' for r in daily),'boundaries':protocol.get('hard_boundaries',[])}
