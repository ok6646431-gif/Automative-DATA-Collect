...
def topic_candidates(inv,sig,actions,chems,scope):
    ...
    for m in inv:
        ...
        s=sm[m['metric_id']]
        if m['comparability']=='TREND_ELIGIBLE' and s['signal_type'] not in NON_ACTIONABLE_SIGNALS: g[(m['canonical_site_id'],m['site_name'],m['domain'])].append((m,s))
    out=[]
