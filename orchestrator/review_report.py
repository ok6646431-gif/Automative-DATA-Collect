...
def select_deep_topics(topics,xmap):
    return [t for t in topics if t.get('candidate_state')=='DEEP_DIVE_CANDIDATE' or xmap.get(t.get('topic_id'),{}).get('review_state')=='FOUR_LAYER_READY']
...
def source_empty_message(state,label,no_evidence):
    return {
        'SOURCE_UNAVAILABLE':f'{label} 수집 실패. 근거 부재로 해석하지 않으며 재수집이 필요함.',
        'SOURCE_PARTIAL':f'{label} 일부 수집 실패. 미수집 범위를 확인하기 전에는 관련 근거가 없다고 판단하지 않음.',
        'SOURCE_NOT_CONFIRMED':f'{label} 수집 가능 여부가 확인되지 않음. 수집상태를 먼저 검증해야 함.',
...
def build_review_report(package_root, render_pdf=True):
    ...
    sm={r.get('metric_id'):r for r in sig}; am={r.get('action_id'):r for r in actions}; evm={r.get('evidence_id'):r for r in layers}; xmap={r.get('topic_id'):r for r in cross}; deep=select_deep_topics(topics,xmap)
    ...
    avrows=''.join(f'<tr><td>{esc(r.get("source_key"))}</td><td>{esc(r.get("evidence_family"))}</td><td>{esc(r.get("availability_state"))}</td><td>{esc(r.get("collection_status"))}</td><td>{esc(r.get("available_count"))}/{esc(r.get("declared_count"))}</td></tr>' for r in availability)
    action_source_state=cross_summary.get('company_action_source_state','-'); industry_source_state=cross_summary.get('industry_reference_source_state','-'); future_source_state=cross_summary.get('future_direction_source_state','-')
    domain_html=[]
    for d in DOMAIN_ORDER:
        ...
        years=sorted(set(r.get('year') for r in dacts if r.get('year'))); labels=[f'{r.get("site_name")} · {r.get("metric")} ({sm.get(r.get("metric_id"),{}).get("signal_type")})' for r in dmain[:8]]
        de=[r for r in layers if r.get('domain')==d and r.get('layer') in {'COMPANY_ACTION','FUTURE_DIRECTION'}]; fut=rank_evidence(de,[d],'FUTURE_DIRECTION',2)
        domain_html.append(f'''<section class="domain"><h3>{DOMAIN_KO[d]}</h3><div class="frame"><b>관리구조를 읽는 순서</b> {esc(DOMAIN_FRAME[d])}</div><div class="stats"><span>가용 지표 <b>{len(dinv)}</b></span><span>장기 Signal <b>{len(dmain)}</b></span><span>관리 Action <b>{len(dacts)}</b></span><span>Action 연도 <b>{esc(', '.join(years) or '-')}</b></span></div><p><b>현재 데이터가 말하는 것:</b> {esc('; '.join(labels[:5]) if labels else '장기 변화 후보가 없거나 현재 공개자료만으로 상세선정 근거가 부족함')}</p><p class="caveat"><b>해석 경계:</b> {esc(DOMAIN_CAVEAT[d])}</p>{evidence_card('회사 미래방향/공개계획 참고',fut,source_empty_message('SOURCE_PARTIAL' if future_source_state=='PARTIAL' else ('SOURCE_UNAVAILABLE' if future_source_state=='UNAVAILABLE' else ''),'회사 문서','현재 수집자료에서 관련 미래방향 근거를 확인하지 못함.'))}</section>''')
    ...
    for y in sorted(by_year):
        ...
        top=sorted(rows,key=inv_key,reverse=True)[:4]; li=''.join(f'<li><b>{esc(a.get("site_name"))}</b> · {esc(a.get("action_name"))} <span class="src">{money(a.get("investment_million_krw"))} 백만원</span></li>' for a in top)
    ...
    for idx,t in enumerate(deep,1):
        ...
        for s in signal_rows[:5]:
            ...
            values=' · '.join(f'{y}:{nice(v)}' for y,v in sorted(vals.items(),key=lambda z:int(z[0]))); sig_cards.append(f'<div class="signal"><b>{esc(s.get("metric"))}</b><span class="tag">{esc(s.get("signal_type"))}</span>{sparkline(vals)}<div class="vals">{esc(values)}</div></div>')
        if t.get('domain')=='CHEMICALS':
            cmain=[c for c in chems if c.get('display_level')=='MAIN'][:10]; rows=''.join(f'<tr><td>{esc(c.get("chemical"))}</td><td>{esc(c.get("years"))}</td><td>{nice(c.get("release_kg"))}</td><td>{nice(c.get("transfer_kg"))}</td><td>{esc(c.get("regulatory_hazard_flags"))}</td></tr>' for c in cmain); sig_cards=[f'<table><thead><tr><th>물질</th><th>반복연도</th><th>배출 kg</th><th>이동 kg</th><th>규제·유해 플래그</th></tr></thead><tbody>{rows}</tbody></table>']
        ...
        act_empty=source_empty_message(x.get('company_action_evidence_state'),'ENV-INFO/관리활동','수집 가능한 ENV-INFO에서 같은 사업장·영역의 공개 Action 근거를 확인하지 못함.')
        act_html=''.join(f'<li><b>{esc(a.get("year"))}</b> · {esc(a.get("action_name"))} - {esc(a.get("description"))}<span class="src">ENV-INFO · {money(a.get("investment_million_krw"))} 백만원</span></li>' for a in act_rows) or f'<li>{esc(act_empty)}</li>'
        industry=[evm[i] for i in split_ids(x.get('industry_reference_evidence_ids')) if i in evm]; future=[evm[i] for i in split_ids(x.get('future_direction_evidence_ids')) if i in evm]; industry=rank_evidence(industry,terms,'INDUSTRY_TECHNICAL',4); future=rank_evidence(future,terms,'FUTURE_DIRECTION',3)
        industry_empty=source_empty_message(x.get('industry_evidence_state'),'기술자료','수집 가능한 기술자료에서 현재 주제와 연결되는 근거를 확인하지 못함.')
        future_empty=source_empty_message(x.get('future_direction_evidence_state'),'회사 문서/미래방향','수집 가능한 회사 문서에서 현재 주제와 연결되는 미래방향 근거를 확인하지 못함.')
        qs=[q for q in questions if q.get('review_id')==x.get('review_id')][:4]; qhtml=''.join(f'<li>{esc(q.get("question"))}<span class="src">필요근거: {esc(q.get("needed_evidence"))}</span></li>' for q in qs) or '<li>생산량·유량·적용기준·운전조건 등 비공개/현장 정보가 필요할 수 있음.</li>'
        deep_html.append(f'''<section class="deep"><h2>Deep Dive {idx}. {esc(t.get('site_name'))} - {esc(DOMAIN_KO.get(t.get('domain'),t.get('domain')))}</h2><p class="lead"><b>왜 보는가:</b> {esc(t.get('why_review'))} <span class="tag">{esc(x.get('review_state') or t.get('candidate_state'))}</span></p><p class="muted"><b>Action:</b> {esc(x.get('company_action_evidence_state') or '-')} / {esc(x.get('company_action_source_state') or '-')} · <b>산업기술:</b> {esc(x.get('industry_evidence_state') or x.get('industry_semantic_ready') or '-')} / {esc(x.get('industry_source_state') or '-')} · <b>미래방향:</b> {esc(x.get('future_direction_evidence_state') or '-')} / {esc(x.get('future_direction_source_state') or '-')}</p><h3>1) 실제 공개데이터에서 관찰된 변화</h3>{''.join(sig_cards)}<h3>2) 같은 사업장/영역에서 공개된 관리활동</h3><ul>{act_html}</ul><div class="grid2">{evidence_card('3) 산업 기술자료: 왜 이 주제를 보는가',industry,industry_empty)}{evidence_card('4) 회사가 밝힌 미래방향',future,future_empty)}</div><h3>5) 여기서 멈춰야 하는 지점</h3><p class="caveat">{esc(t.get('limitations') or x.get('limitations') or '')}</p><h3>6) 다음에 확인할 질문</h3><ul>{qhtml}</ul></section>''')
    ...
    html_text=f'''<!doctype html><html lang="ko"><head><meta charset="utf-8"><style>@page{{size:A4;margin:13mm 12mm 15mm}} body{{font-family:sans-serif;color:#172033;font-size:9.3pt;line-height:1.48}} h1{{font-size:22pt;margin:0 0 4mm}} h2{{font-size:15pt;margin:7mm 0 3mm;border-bottom:2px solid #17365D;padding-bottom:1.5mm}} h3{{font-size:11.5pt;margin:4mm 0 1.5mm;color:#17365D}} h4{{margin:2mm 0 1mm}} p{{margin:1.2mm 0}} ul{{margin:1.2mm 0 2mm;padding-left:5mm}} li{{margin:.8mm 0}} table{{width:100%;border-collapse:collapse;font-size:8.2pt;margin:2mm 0}} th,td{{border:1px solid #c7ced8;padding:1.4mm;vertical-align:top}} th{{background:#eef2f7}} .cover{{min-height:235mm;display:flex;flex-direction:column;justify-content:center}} .kicker{{font-size:10pt;color:#5b6575;letter-spacing:.04em}} .subtitle{{font-size:12pt;color:#465164;margin:3mm 0 8mm}} .box,.domain,.deep{{border:1px solid #d4dbe5;border-radius:6px;padding:4mm;margin:3mm 0;break-inside:avoid}} .domain{{break-inside:auto}} .deep{{break-before:page;border:none;padding:0}} .grid2{{display:grid;grid-template-columns:1fr 1fr;gap:3mm}} .stats{{display:flex;gap:2mm;flex-wrap:wrap;margin:2mm 0}} .stats span,.tag{{background:#eef3f8;border-radius:12px;padding:1mm 2mm;font-size:8pt}} .frame{{background:#f5f7fa;padding:2mm 3mm;border-left:3px solid #17365D}} .caveat{{background:#fff8e7;border-left:3px solid #d49a00;padding:2mm 3mm}} .muted,.src{{color:#697587;font-size:7.6pt}} .src{{display:block;margin-top:.4mm}} .evi{{border:1px solid #d9e0e8;padding:2.5mm;break-inside:avoid}} .signal{{display:inline-block;width:47%;vertical-align:top;border:1px solid #d9e0e8;padding:2mm;margin:1mm;break-inside:avoid}} .spark{{width:100%;height:48px;margin-top:1mm}} .spark polyline{{fill:none;stroke:#17365D;stroke-width:2}} .spark circle{{fill:#17365D}} .spark text{{font-size:8px;fill:#667}} .vals{{font-size:7.5pt;color:#596577;word-break:break-all}} .year{{border-left:3px solid #8fa6bf;padding-left:3mm;margin:3mm 0;break-inside:avoid}} .year h4 span{{font-weight:normal;font-size:8pt;color:#697587}} .lead{{font-size:10pt}} .pagebreak{{break-before:page}}</style></head><body><section class="cover"><div class="kicker">AI-assisted Environmental Management Review</div><h1>{esc(report_name)} 환경관리 검토보고서</h1><div class="subtitle">공개근거 기반 · 요청범위 {esc(scope.get('label') or scope.get('mode'))} · 자동 생성 검증형 프로토타입</div><div class="box"><b>이 보고서의 목적</b><p>기업 환경성과를 평가하거나 현장 원인을 단정하는 문서가 아니다. 비전문가가 공개자료를 통해 <b>무엇을 관리하는 회사인지, 어떤 변화가 보이는지, 무엇을 더 공부해야 하는지</b>를 근거를 따라 이해하도록 돕는 검토자료다.</p></div><div class="stats"><span>요청범위 지표 {review_summary.get('metric_inventory_in_scope','-')}</span><span>관리 Action {review_summary.get('management_actions_in_scope','-')}</span><span>검토주제 {len(topics)}</span><span>Deep Dive {len(deep)}</span><span>문서 스캔 {cross_summary.get('document_semantics',{}).get('pages_scanned','-')}p</span><span>Action Source {esc(action_source_state)}</span><span>산업기술 Source {esc(industry_source_state)}</span><span>미래방향 Source {esc(future_source_state)}</span></div><p class="muted">자동생성 결과는 원자료·페이지 근거와 함께 검토해야 하며, AI가 제시한 연관성은 인과관계를 뜻하지 않는다.</p></section><h2>1. 먼저 머릿속에 넣을 구조</h2><div class="box"><p><b>회사/사업범위</b> → <b>사업장</b> → <b>환경영역</b> → <b>실제 공개값</b> → <b>회사 관리활동</b> → <b>산업 기술근거</b> → <b>미래방향</b> → <b>추가확인</b></p><p>각 층은 독립 근거로 유지한다. 시간적으로 겹친다는 이유만으로 ‘시설투자 때문에 수치가 개선됐다’고 연결하지 않는다. 또한 <b>자료를 찾지 못한 것</b>과 <b>자료 수집에 실패한 것</b>을 구분한다.</p></div><h3>요청 사업장</h3><table><thead><tr><th>사업장</th><th>공식 주소</th><th>분석 ID</th></tr></thead><tbody>{site_rows}</tbody></table><h3>공개자료 범위</h3><table><thead><tr><th>Source</th><th>요청범위 연도</th><th>연도 수</th></tr></thead><tbody>{cvrows}</tbody></table><p class="muted">화학물질통계처럼 조사주기가 다른 Source는 동일 연도수로 맞추지 않는다.</p><h3>Source 수집상태</h3><table><thead><tr><th>Source</th><th>근거군</th><th>상태</th><th>수집결과</th><th>가용/선언</th></tr></thead><tbody>{avrows}</tbody></table><p class="muted">UNAVAILABLE/PARTIAL은 ‘관련 근거가 없다’가 아니라 수집상태가 불완전하다는 뜻이다.</p><h2>2. 환경관리 전체 지도</h2>{''.join(domain_html)}<h2>3. 2020-2024 관리시설·개선활동 Timeline</h2><p>ENV-INFO에 회사가 공개한 투자·기술도입·개선활동을 요청 사업장 기준으로 모았다. 투자액 합계는 공개 항목의 단순 합이며 성과평가 지표가 아니다.</p>{''.join(year_blocks)}<h2>4. 근거가 겹치는 우선 검토주제</h2><p>전체 {len(topics)}개 검토주제 중 현재 규칙상 상세검토 후보는 {len(deep)}개다. 초기 단계의 <b>다년도 Signal + 같은 사업장/영역 Action + 독립 Source 중첩</b> 또는 후속 단계의 <b>OBSERVED + COMPANY_ACTION + INDUSTRY_TECHNICAL + FUTURE_DIRECTION 4개 근거층 완비</b>를 근거로 포함한다. 이는 중요도·위험도 순위나 인과판정이 아니다.</p>{''.join(deep_html)}<h2 class="pagebreak">5. 이 보고서를 읽을 때 지켜야 할 경계</h2><ul>{bhtml}</ul><div class="box"><b>결론 대신 남겨야 하는 것</b><p>[FACT] 원자료에서 직접 확인한 사실 / [AI REVIEW] 근거가 겹쳐 더 볼 가치가 있는 후보 / [TO VERIFY] 공개자료만으로 부족해 현장·허가·운전정보가 필요한 질문을 구분한다.</p></div></body></html>'''
    ...
    if render_pdf:
        ...
        if browser:
            ...
            cp=subprocess.run(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=90); pdf_ok=valid_pdf(pdf_path)
    ...
    summary={'schema_version':'1.2','report_html':html_path.name,'report_pdf':pdf_path.name if pdf_ok else None,'evidence_xlsx':xlsx_path.name if xlsx_path and xlsx_path.exists() else None,'deep_dive_topics':len(deep),'deep_dive_selection_basis':'EARLY_DEEP_DIVE_OR_FOUR_LAYER_READY','scope_sites':len(sites),'company_action_source_state':action_source_state,'industry_reference_source_state':industry_source_state,'future_direction_source_state':future_source_state,'pdf_ok':pdf_ok,'pdf_error':pdf_error,'principle':'The report organizes evidence for study; source collection failure is distinct from evidence absence across action, industry-technical and future-direction layers; it does not automatically judge environmental performance, compliance, risk or causality.'}
